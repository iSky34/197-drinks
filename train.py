...
class LitTransformer(LightningModule):
    def __init__(self, num_classes=10, lr=0.001, max_epochs=30, depth=12, embed_dim=32,
...
#     def __init__(self, batch_size=64, num_workers=32, patch_num=4, **kwargs):
#         super().__init__()
#         self.batch_size = batch_size
...
class KWSDataModule(LightningDataModule):
    def __init__(self,path, batch_size, num_workers=32, patch_num=4, n_fft=512,n_mels=64, win_length=None, hop_length=252, class_dict={}, **kwargs):
        ...
        self.class_dict=class_dict
#    def prepare_data(self):
#        self.train_set = KWSModel(root='./data', train=True,
#                                 download=True, transform=torchvision.transforms.ToTensor())
#        self.test_set = KWSModel(root='./data', train=False,
#                                download=True, transform=torchvision.transforms.ToTensor())
    def prepare_data(self):
     #   self.train_set = KWSModel(root='./data', train=True,
     ##                            download=True, transform=torchvision.transforms.ToTensor())
    ...
    def collate_fn(self, batch):
        ...
        for sample in batch:
            ...
            # mel from power to db
            mels.append(ToTensor()(librosa.power_to_db(self.transform(waveform).squeeze(0).numpy(), ref=np.max)))
...
if __name__ == "__main__":
    ...
    datamodule = KWSDataModule(path=args.path,batch_size=64,
                            patch_num=16, 
                            num_workers=2,class_dict=CLASS_TO_IDX)
...
model = model.load_from_checkpoint("/content/data/speech_commands/SpeechCommands/speech_commands_v0.02/checkpoints/kws.pt.ckpt")
model.eval()
script = model.to_torchscript()

    # save for use in production environment
model_path = os.path.join(
                            "/content/kwstransform.pt")
...
mels=[]
            ...
            # mel from power to db
mels.append(ToTensor()(librosa.power_to_db(transform(waveform).squeeze(0).numpy(), ref=np.max)))
    #mels.append(self.transform(waveform))
#     labels.append(torch.tensor(self.class_dict[label]))
#     labels.append(torch.tensor(self.class_dict[label]))
#    wavs.append(waveform)
...
print(mels.size())
mel = rearrange(mels, 'b c (h) (p2 w) -> b (p2) (c h w)', p2=16)
print(mels.size())
